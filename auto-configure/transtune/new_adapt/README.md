...
本目录是重构后的实现，**不依赖** `auto-configure/transtune/adapt` 目录中的旧代码。
...
```bash
bash auto-configure/transtune/new_adapt/run_drift_benchmark.sh milvus-single-node milvus-p10 random-100
```

使用 GloVe（h5）示例：

```bash
bash auto-configure/transtune/new_adapt/run_drift_benchmark.sh milvus-single-node milvus-p10 glove-100-angular
```
...
RUN_TAG=exp-a bash auto-configure/transtune/new_adapt/run_drift_benchmark.sh milvus-single-node milvus-p10 random-100
```
...
- 脚本目录 `auto-configure/transtune/new_adapt/` 本身不产生运行产物
...
BENCH=/path/to/TransTune/vector-db-benchmark-master
...
# sudo rm -rf /path/to/docker/volumes/{etcd,minio,milvus}/*
```
...
```bash
rm -rf /path/to/TransTune/vector-db-benchmark-master/datasets/new_adapt/drift-*
```
