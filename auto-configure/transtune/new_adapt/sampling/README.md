...
```bash
bash auto-configure/transtune/new_adapt/sampling/run_sampling_benchmark.sh \
...
| `VENV_PATH` | `/path/to/venv` | Python 虚拟环境路径 |
...
```bash
python3 auto-configure/transtune/new_adapt/sampling/generate_sampled_dataset.py \
...
```bash
cd auto-configure/transtune/new_adapt/sampling
