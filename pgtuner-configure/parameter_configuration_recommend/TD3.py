...
class TD3(object):
    ...
    def _sample_batch(self):
        ...
        valid = [(i, x) for i, x in zip(idx, batch) if x is not None]
        if len(valid) < self.batch_size:
            raise RuntimeError(
                f"Replay memory returned invalid samples: got {len(valid)}/{self.batch_size} valid transitions."
            )
        idx = [i for i, _ in valid]
        batch = [x for _, x in valid]
    ...
    def add_sample(self, states, actions, rewards, next_states, terminates):
        ...
        for i in range(batch):
            ...
            if not (
                np.isfinite(np.asarray(state)).all()
                and np.isfinite(np.asarray(action)).all()
                and np.isfinite(np.asarray(reward)).all()
                and np.isfinite(np.asarray(next_state)).all()
            ):
                continue
