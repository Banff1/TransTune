# -*- coding: utf-8 -*-
"""
Train PCR (TD3) when QPP uses VDTuner prior-shaped inputs.

Mutates ``Args.args`` ``n_states`` / ``n_actions`` to ``2*K+6`` and ``K``.
Requires prior QPP checkpoint and ``feature_standard_vdtuner_prior*.npz`` from ``fit_feature_scaler_prior.py``.
"""
...
from utils import Logger, time_end, time_start  # noqa: E402
...
def _baseline_mask(df: pd.DataFrame, baseline: dict) -> pd.Series:
    ...
    for k, v in baseline.items():
        if k not in df.columns:
            raise ValueError(f"baseline key {k} not in train CSV")
...
def _extra_cli():
    ...
    p.add_argument("--prior-config", type=str, required=True, help="Unified JSON: knobs + tune_knobs + baseline + …")
    p.add_argument("--knob-json", type=str, default="")
    p.add_argument(
        "--feature-standard-npz",
        type=str,
        default="",
        help="Path to feature scaler npz (default: query_performance_predict/scaler_paras/feature_standard_vdtuner_prior.npz)",
    )
...
def main() -> None:
    ...
    print(f"[train_vdtuner_prior] K={k} n_states={n_states} n_actions={n_actions}")

    current_directory = str(_PCR)
    parent_directory = str(_PCR.parent)
    ...
    predict_model_save_path = os.path.join(
        parent_directory,
        "query_performance_predict/model_checkpoints/{}_{}_{}_{}_checkpoint.pth".format(
            args_p.dipredict_layer_sizes, args_p.dipredict_n_epochs, args_p.dipredict_batch_size, args_p.dipredict_lr
        ),
    )
    if xa.feature_standard_npz:
        standard_path = xa.feature_standard_npz
    else:
        standard_path = os.path.join(parent_directory, "query_performance_predict/scaler_paras/feature_standard_vdtuner_prior.npz")
    ...
    if len(def_df) == 0:
        raise RuntimeError("No train row matches prior-config baseline; check values against CSV")
    ...
    print("----------------start training (VDTuner prior PCR)...----------------")
    ...
    for episode in tqdm(range(args_r.epoches), total=args_r.epoches):
        ...
        if train_step > 0:
            ...
            episode_closs[episode] = 0.0
            episode_aloss[episode] = 0.0
        ...
        if (episode + 1) % 100 == 0:
            logger.info(
                "[Episode: {}][Average] step: {}s env step: {}s add step: {}s train step: {}s action time: {}s".format(
                    ...
                    np.mean(add_step_times),
                    np.mean(train_step_times),
    ...
    logger.info("Training time: {}s".format(time.time() - start_time))
