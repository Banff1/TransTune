...
class IndexEnv(object): 
    def __init__(self, num_dataset, default_performance, target_rec_lis, args_r, args_p, predict_model_save_path, standard_path, dv):
        ...
        _bef = int(getattr(args_r, "baseline_efsearch", 10))
        self.default_paras = np.tile(np.array([[20, 4, _bef]]), (self.target_rec.shape[0], 1))
    ...
    def _get_index_performance(self, feature_input):
        ...
        # Keep counts positive/stable to avoid NaN/Inf rewards and replay priorities.
...
class IndexEnv_nsg(object):
    ...
    @staticmethod
    def _get_performance_improvement(last_index_performance, current_index_performance):
        denom = np.where(np.abs(last_index_performance[:, 1]) < 1e-12, 1e-12, last_index_performance[:, 1])
        st_counts_dec = (last_index_performance[:, 1] - current_index_performance[:, 1]) / denom
        st_counts_dec = np.nan_to_num(st_counts_dec, nan=0.0, posinf=0.0, neginf=0.0)
        return st_counts_dec
