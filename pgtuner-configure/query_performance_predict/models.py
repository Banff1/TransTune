...
class Direct_Predict_MLP(nn.Module):
    def __init__(self, layer_sizes, final_relu=False):
        ...
        self.register_buffer("weights", torch.tensor([[1.0, 5.0, 20.0]], dtype=torch.float32))
    ...
    def calculate_loss(self, inp, label):
        w = self.weights.to(dtype=inp.dtype, device=inp.device)
        weighted_loss = self.loss(inp, label) * w
...
class Direct_Predict_MLP_nsg(nn.Module):
    def __init__(self, layer_sizes, final_relu=False):
        ...
        self.register_buffer("weights", torch.tensor([[1.0, 20.0]], dtype=torch.float32))
